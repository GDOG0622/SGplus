...
/** Lets the settings panel refresh its counters without importing this module's internals. */
export function onInventoryChanged(listener) {
...
function notifyInventoryChanged() {
    for (const listener of inventoryListeners) {
        try {
            ...
            console.warn(`[${DISPLAY_NAME}] 资源清单回调失败`, error);
        }
    }
}
...
export function applySmartGrouping(adapter, { announce = true, items = null } = {}) {
...
function ensureManager() {
    ...
    if (!mask) {
        ...
        mask.id = MANAGER_ID;
        mask.innerHTML = `<button type="button" class="srg-manager-backdrop" data-srg-manager-backdrop aria-label="关闭分组管理器背景" tabindex="-1"></button><div class="srg-manager" role="dialog" aria-modal="true" aria-label="${DISPLAY_NAME}"></div>`;
...
export function openManager(adapterId = '') {
...
export function getManagerAdapters() {
...
export function getManagerItems(adapter) {
    const candidates = [
        adapter,
...
export function exportGroupingData() {
    const data = {
        ...
        regexGroups: settings.regexGroups,
    };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = `sgplus-groups-${new Date().toISOString().slice(0, 10)}.json`;
...
export async function importGroupingData(file, { onImported } = {}) {
    try {
        ...
        if (data.regexGroups && typeof data.regexGroups === 'object' && !Array.isArray(data.regexGroups)) {
            settings.regexGroups = clone(data.regexGroups);
        }
        scheduleSave();
...
function injectMenuEntry() {
    ...
    item.tabIndex = 0;
    item.innerHTML = `<span class="srg-menu-label">${DISPLAY_NAME}</span>`;
...
let ensureSettingsPanel = () => false;

/** The settings panel lives in index.js so it can also expose prompt-entry options. */
export function setSettingsPanelFactory(factory) {
    ensureSettingsPanel = factory;
}
...
export function refreshAllMounts() {
...
function bindGlobalUiEvents() {
    const handleDocumentPointerDown = event => {
        if (getHostModalDepth() > 0 || event.target?.closest?.('.popup, dialog[open]')) return;
